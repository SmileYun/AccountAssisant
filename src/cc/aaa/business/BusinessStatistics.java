...
public class BusinessStatistics extends BusinessBase {
	...
	public BusinessStatistics(Context pContext) {
		...
		mBusinessAccountBook = new BusinessAccountBook(pContext);
	}

	public String getPayoutUserIDByAccountBookID(int pAccountBookID) {
		String _Result = "";
		List<ModelStatistics> _ListModelStatisticsTotal = getPayoutUserID(" And AccountBookID = " + pAccountBookID);
		...
		for (int i = 0; i < _ListModelStatisticsTotal.size(); i++) {
			...
			if (_ModelStatistics.getPayoutType().equals("个人")) {
				...
				if (_ModelStatistics.PayerUserID.equals(_ModelStatistics.ConsumerUserID)) {
					...
					_Result += _ModelStatistics.ConsumerUserID + "应支付给" + _ModelStatistics.PayerUserID
							+ _ModelStatistics.Cost + "元\r\n";
				}
			}
	...
	public List<ModelStatistics> getPayoutUserID(String pCondition) {
		// 得到拆分好的统计信息
		List<ModelStatistics> _ListModelStatistics = getModelStatisticsList(pCondition);
		// 存放按付款人分类的临时统计信息
		...
		for (int i = 0; i < _ListModelStatistics.size(); i++) {
			...
			if (!_CurrentPayerUserID.equals(_ListModelStatistics.get(_NextIndex).PayerUserID) || _NextIndex == i) {
				...
				for (int j = 0; j < _ListModelStatisticsTemp.size(); j++) {
					...
					int _Index = getPostionByConsumerUserID(_ListModelStatisticsTotal, _ModelStatisticsTotal.PayerUserID,
							_ModelStatisticsTotal.ConsumerUserID);
					// 如果已经存在，则开始在原来的数据上进行累加
					if (_Index != -1) {
						_ListModelStatisticsTotal.get(_Index).Cost = _ListModelStatisticsTotal.get(_Index).Cost
								.add(_ModelStatisticsTotal.Cost);
					} else {
						// 否则就是一条新信息，添加到统计数组当中
	...
	private int getPostionByConsumerUserID(List<ModelStatistics> pListModelStatisticsTotal, String pPayerUserID,
			String pConsumerUserID) {
		int _Index = -1;
		for (int i = 0; i < pListModelStatisticsTotal.size(); i++) {
			if (pListModelStatisticsTotal.get(i).PayerUserID.equals(pPayerUserID)
					&& pListModelStatisticsTotal.get(i).ConsumerUserID.equals(pConsumerUserID)) {
				_Index = i;
			}
		}
	...
	private List<ModelStatistics> getModelStatisticsList(String pCondition) {
		// 按支付人ID排序取出消费记录
		List<ModelPayout> _ListPayout = mBusinessPayout.getPayoutOrderByPayoutUserID(pCondition);
		...
		if (_ListPayout != null) {
			...
			for (int i = 0; i < _ListPayout.size(); i++) {
				...
				// 将消费人ID转换为真实名称
				String _PayoutUserName[] = mBusinessUser.getUserNameByUserID(_ModelPayout.getPayoutUserID()).split(",");
	...
	public String ExportStatistics(int pAccountBookID) throws Exception {
		String _Result = "";
		String _AccountBookName = mBusinessAccountBook.GetAccountBookNameByAccountId(pAccountBookID);
		Date _Date = new Date();
		// String _FileName = _AccountBookName + String.valueOf(_Date.getYear())
		// + String.valueOf(_Date.getMonth()) + String.valueOf(_Date.getDay()) +
		// ".xls";
		String _FileName = String.valueOf(_Date.getYear()) + String.valueOf(_Date.getMonth())
				+ String.valueOf(_Date.getDay()) + ".xls";
		...
		for (int i = 0; i < _ListModelStatisticsTotal.size(); i++) {
			...
			if (_ModelStatistics.getPayoutType().equals("个人")) {
				...
				if (_ModelStatistics.PayerUserID.equals(_ModelStatistics.ConsumerUserID)) {
					...
					_Info = _ModelStatistics.ConsumerUserID + "应支付给" + _ModelStatistics.PayerUserID + _ModelStatistics.Cost
							+ "元";
				}
			}

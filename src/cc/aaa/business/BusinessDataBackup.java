...
import cc.aaa.utility.FileUtil;
	...
	public boolean databaseBackup(Date p_Backup) {
		...
		try {
			...
			if (_SourceFile.exists()) {
				...
				FileUtil.cp("/data/data/" + getContext().getPackageName() + "/databases/GoDutchDataBase",
						"/sdcard/GoDutch/DataBaseBak/GoDutchDataBase");
			}
	...
	public boolean databaseRestore() {
		...
		try {
			...
			if (_DatabaseBackupDate != 0) {
				FileUtil.cp("/sdcard/GoDutch/DataBaseBak/GoDutchDataBase",
						"/data/data/Mobidever.GoDutch/databases/GoDutchDataBase");
			}
